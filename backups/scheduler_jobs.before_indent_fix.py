...
from services.x_relay import post_with_backoff, relay_enabled
from services.x_templates import preroll_line, holy_minute_line, anchor_line, news_digest_line
...
async def job_toka_preroll(context):
    ...
    try:
        if relay_enabled():
            post_with_backoff(preroll_line(a.get('symbol','—'), f"${(a.get('price') or 0):.4f}"), retries=1)
    except Exception:
        pass
async def job_toka_holy(context):
    ...
    try:
        if relay_enabled():
            post_with_backoff(holy_minute_line(a.get('symbol','—'), f"${(a.get('price') or 0):.4f}", f"{(a.get('change24h_pct') or 0):+.2f}%"), retries=1)
    except Exception:
        pass
try:
        set_last_posted_420()

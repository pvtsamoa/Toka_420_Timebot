...
def get_anchor(token: str) -> Dict[str, str]:
    ...
    try:
        ...
        if pair:
            ...
            ch24 = f"{float(ch24v):+,.2f}%" if ch24v is not None else "n/a"
            vol24v = (pair.get("volume") or {}).get("h24") or pair.get("volume24h")
            vol24 = _fmt_usd(vol24v) if vol24v is not None else "n/a"
    except Exception as e:
        logger.warning("Price fetch failed: %s", e)
    data = {"price": price, "change_24h": ch24, "volume_24h": vol24}

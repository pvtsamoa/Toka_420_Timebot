...
DEFAULT_HUB = 'americas'  # Los Angeles timezone region default

def get_chat_hub(chat_id, default_hub: str = "americas"):
    d = _load()
    row = (d.get(str(chat_id)) or {})
    hub = row.get("hub")
    return (hub or default_hub)

def set_chat_hub(chat_id, hub: str):
    hub = (hub or "").lower().strip()
    if hub not in {"asia","europe","americas","oceania"}:
        raise ValueError("hub must be one of: asia, europe, americas, oceania")
    d = _load()
    row = (d.get(str(chat_id)) or {})
    row["hub"] = hub
    d[str(chat_id)] = row

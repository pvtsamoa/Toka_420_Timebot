...
def _load_lines(p: Path, fallback: List[str]) -> List[str]:
    try:
        if p.exists():
...
# Load blessing/joke banks with safe fallbacks
_BLESSINGS = _load_lines(
    Path("media/blessings.json"),
    ["Ua tafe le vasa, ae tumau le ma‘anunu — the tide moves, but the anchor holds."]
)
_JOKES = _load_lines(
    Path("media/jokes.json"),
    ["Stoner TA: higher highs, higher vibes."]
)

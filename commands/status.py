...
def _count_jobs(context):
    ...
    if sched:
        try:
            jobs = sched.get_jobs()
            hub_jobs = [j for j in jobs if (getattr(j, "id", "") or "").startswith("hub420:")]
    ...
    if jq:
        try:
            jobs = jq.jobs()
            hub_jobs = [j for j in jobs if (getattr(j, "name", "") or "").startswith("hub420:")]
    ...
    if jq:
        try:
            ...
            next_times = [
                j.next_t
                for j in jobs
                if j and getattr(j, "name", "") and (j.name or "").startswith("hub420:") and getattr(j, "next_t", None)
            ]
            if not next_times:
...
"""
        ...
        await update.message.reply_text("⚠️ Error generating status. Try again later.", parse_mode="Markdown")
